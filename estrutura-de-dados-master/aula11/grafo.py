class Arco():
    ...
    def oposto(self, x):
        if x != self.vertices[0]: return self.vertices[0]
        else: return self.vertices[1]
...
class Grafo():
    ...
    def adicionar_vertice(self, nomeVertice):
        self.vertice = self.vertice + (nomeVertice,)

    def adicionar_arco(self, nomeArco):
        self.arco = self.arco + (nomeArco,)

    def arcos(self, nomeArco):
        resposta = tuple()
        for x in self.arco:
            if nomeArco in x.vertices:
                resposta = resposta + (x,)
        return resposta

    def adjacencias(self, nomeAdjacencia):
        resposta = tuple()
        for x in self.arco:
            if nomeAdjacencia in x.vertices:
                resposta = resposta+(x.oposto(nomeAdjacencia),)
        return resposta

    def caminho(self, vertice1, vertice2):
        resposta = []

        if vertice1 is vertice2:
            resposta.append(vertice1)
            return resposta
        elif self.arcos(vertice1) == tuple():
            return resposta
        else:
            while(vertice1 is not vertice2):
                if self.arcos(vertice1) == tuple(): break
                for i in self.arcos(vertice1):
                    if i.oposto(vertice1) not in resposta:
                        resposta.append(vertice1)
                        vertice1 = i.oposto(vertice1)
                        break
            resposta.append(vertice1)
            return resposta

    def calcular_melhores_caminhos_partindo_de(self, vertece):
        resposta = {}
        visitados, faltam = [] , []
        resposta[vertece] = (0, [vertece])
        visitados.append(vertece)
        for posicao in range(len(self.arco)):
            if vertece in self.arco[posicao].vertices: faltam.append((self.arco[posicao].valor, self.arco[posicao]))
        while faltam is not None:
            for x in faltam:
                if x[1].vertices[0] in visitados and x[1].vertices[1] in visitados: faltam.remove(x)
            faltam.sort(key = lambda arcos: arcos[0])#lambda - Filtro
            if faltam: arco = faltam.pop(0)
            else: return resposta
            if arco[1].vertices[0] in visitados:
                visitados.append(arco[1].vertices[1])
                ver = arco[1].vertices[0]
            else:
                visitados.append(arco[1].vertices[0])
                ver = arco[1].vertices[1]
            resposta[visitados[-1]] = arco[0], resposta[ver][1] + [arco[1].valor] + [visitados[-1]]
            for posicao in range(len(self.arco)):
                if visitados[-1] in self.arco[posicao].vertices and (not self.arco[posicao].vertices[0] in visitados or not self.arco[posicao].vertices[1] in visitados):
                    resp = (resposta[visitados[-1]][0] + self.arco[posicao].valor, self.arco[posicao])
                    faltam.append(resp)
        return resposta
